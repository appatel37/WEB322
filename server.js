 ...
 *
 *  Name: Aryan Pareshbhai Patel Student ID: 133889212 Date: 06/04/2023
 *
 *  Online (Cyclic) Link: https://worried-red-elk.cyclic.app/
 *
 ********************************************************************************/
...
const auth_data = require("./auth-service.js");
...
app.use(
  client_sessions({
    cookieName: "session",
    secret: "aryan_web322_Assignment",
    duration: 2 * 60 * 1000,
...
app.get("/posts/add", ensureLogin, (req, res) => {
  blogData
    .getCategories()
    .then((data) => {
      if (data.length > 0) res.render("addPost", { categories: data });
      else res.render("addPost", { categories: [] });
    })
    .catch((err) => {
...
app.post("/categories/add", ensureLogin, (req, res) => {
  console.log("Aryan Patel" + req.body);
  blogData
...
app.get("/categories/delete/:id", ensureLogin, (req, res) => {
  blogData
    ...
    .catch(() => {
      res.statusCode(500).send("Unable to Remove category");
    });
});
...
app.get("/post/delete/:id", (req, res) => {
  blogData
    ...
    .catch(() => {
      res.statusCode(500).send("Unable to Remove Post");
    });
});
